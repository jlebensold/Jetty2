describe 'App.Models.Authority', ->
  ...
  it 'should be able to determine a root node', ->
    ...
    expect(auth.first().children().last().is_root_node()).toBeFalsy()
  ...
  it 'should be able to navigate to root node', ->
    ...
    auth.fetch()
    @server.respond()
    expect(auth.first().children().last().root_node().id).toEqual(auth.first().id)
  ...
  it 'should be able to search for a node within the children of a tree', ->
    ...
    auth.fetch()
    @server.respond()
    canada = auth.first().children().models[1].children().first().children().first()
  ...
  it 'should be able to remove a node and its children from the tree', ->
    ...
    @server.respond()
    canada = auth.first().children().models[1].children().first().children().first()
    expect(auth.first().children().models[1].children().first().children().length).toEqual(1)
    auth.first().remove canada    
    expect(auth.first().children().models[1].children().first().children().length).toEqual(0)
  ...
  it 'should be able to move stuff around the tree', ->
    ...
    expect(auth.children().models[1].children().length).toEqual(5)
    
    canada = auth.children().models[1].children().first().children().first()
    auth.remove canada
    auth.children().last().attach canada

    expect(auth.children().last().children().length).toEqual(1)
